...
def aug(image):
    ...
    img3 = aug.apply(image_for_augmentation, alpha=alpha, sigma=sigma, alpha_affine=alpha_affine)

    distort_limit = 0.2 
    shift_limit = 0.15 
    aug = OpticalDistortion(p=1)
    image_for_augmentation = image.copy()
    img4 = aug.apply(image_for_augmentation, k=distort_limit, dx=shift_limit, dy=shift_limit)
    ...
    return torch.tensor(np.stack([img1,img2,img3,img4,img5,img6])).to(device)
